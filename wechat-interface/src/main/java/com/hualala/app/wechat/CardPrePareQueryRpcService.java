...
public interface CardPrePareQueryRpcService {
...
//    CardSyncResData syncCouponInfo(CardSyncReqData cardSyncReqData);
    ...
    @RpcMethod(description = "查询多个会员卡数据")
    CardBaseInfoResDataList queryBaseInfoList(CardQuery cardQuery);
    /**
     * 查询advancedInfo
     * 查一个
     */
...
//    }
    ...
    class CouponResData extends ResultInfo {
        ...
        @Protocol(fieldType = FieldType.STRING, order = 3, description = "优惠券专用，填写优惠详情。")
        private String defaultDetail;
        //       deal_detail	        是	string(3072)	双人套餐\n -进口红酒一支。\n孜然牛肉一份。	团购券专用，团购详情。
        @Protocol(fieldType = FieldType.STRING, order = 4, description = "团购券专用，填写团购详情。")
        private String dealDetail;
        //        least_cost	        是	int	10000	    代金券专用，表示起用金额（单位为分）,如果无起用门槛则填0。
        @Protocol(fieldType = FieldType.STRING, order = 5, description = "代金券专用，表示起用金额（单位为分）,如果无起用门槛则填0。")
        private Integer leastCost;
        //        reduce_cost	        是	int	10000	    代金券专用，表示减免金额。（单位为分）
        @Protocol(fieldType = FieldType.STRING, order = 6, description = "代金券专用，表示减免金额。（单位为分）")
        private Integer reduceCost;
        //        discount	            是	int	30	        折扣券专用，表示打折额度（百分比）。填30就是七折。
        @Protocol(fieldType = FieldType.STRING, order = 7, description = "折扣券专用，表示打折额度（百分比）。填30就是七折。")
        private Integer discount;
        //        gift	                是	string(3072)	可兑换音乐木盒一个。	兑换券专用，填写兑换内容的名称。
        @Protocol(fieldType = FieldType.STRING, order = 8, description = "可兑换音乐木盒一个。兑换券专用，填写兑换内容的名称。")
        private String gift;
    ...
    class MemberResData extends ResultInfo {
        ...
        @Protocol(fieldType = FieldType.LONG, order = 2, description = "会员端产生的唯一主键ID")
        private Long cardKey;
        //        discount            	否            	int            	折扣，该会员卡享受的折扣优惠,填10就是九折。
        @Protocol(fieldType = FieldType.INT, order = 3, description = "折扣，该会员卡享受的折扣优惠,填10就是九折。")
        private Integer discount;
        //        background_pic_url        否            	string(128)         商家自定义会员卡背景图，须先调用上传图片接口将背景图上传至CDN，否则报错，卡面设计请遵循微信会员卡自定义背景设计规范  ,像素大小控制在1000像素*600像素以下
        @Protocol(fieldType = FieldType.STRING, order = 4, description = "商家自定义会员卡背景图，须先调用上传图片接口将背景图上传至CDN，否则报错，卡面设计请遵循微信会员卡自定义背景设计规范  ,像素大小控制在1000像素*600像素以下")
        private String backgroundPicUrl;
        //        prerogative            	是
        @Protocol(fieldType = FieldType.STRING, order = 5, description = "会员卡特权说明。")
        private String prerogative;
        //        auto_activate            	否            	bool            	设置为true时用户领取会员卡后系统自动将其激活，无需调用激活接口，详情见自动激活。
        @Protocol(fieldType = FieldType.BOOL, order = 6, description = "设置为true时用户领取会员卡后系统自动将其激活，无需调用激活接口，详情见自动激活。")
        private boolean autoActivate;
        //        wx_activate            	否            	bool            	设置为true时会员卡支持一键开卡，不允许同时传入activate_url字段，否则设置wx_activate失效。填入该字段后仍需调用接口设置开卡项方可生效，详情见一键开卡。
        @Protocol(fieldType = FieldType.BOOL, order = 7, description = "设置为true时会员卡支持一键开卡，不允许同时传入activate_url字段，否则设置wx_activate失效。填入该字段后仍需调用接口设置开卡项方可生效，详情见一键开卡。")
        private boolean wxActivate;
        //        supply_bonus            	是            	bool            	显示积分，填写true或false，如填写true，积分相关字段均为必填。
        @Protocol(fieldType = FieldType.BOOL, order = 8, description = "显示积分，填写true或false，如填写true，积分相关字段均为必填。")
        private boolean supplyBonus;
        //        bonus_url            	否            	string(128)            	设置跳转外链查看积分详情。仅适用于积分无法通过激活接口同步的情况下使用该字段。
        @Protocol(fieldType = FieldType.STRING, order = 9, description = "设置跳转外链查看积分详情。仅适用于积分无法通过激活接口同步的情况下使用该字段。")
        private String bonusUrl;
        //        supply_balance            	是            	bool            	是否支持储值，填写true或false。如填写true，储值相关字段均为必填。
        @Protocol(fieldType = FieldType.BOOL, order = 10, description = "是否支持储值，填写true或false。如填写true，储值相关字段均为必填。")
        private boolean supplyBalance;
        //        balance_url            	否            	string(128)            	设置跳转外链查看余额详情。仅适用于余额无法通过激活接口同步的情况下使用该字段。
        @Protocol(fieldType = FieldType.STRING, order = 11, description = "设置跳转外链查看余额详情。仅适用于余额无法通过激活接口同步的情况下使用该字段。")
        private String balanceUrl;
        //        custom_field1            	否            	JSON结构            	自定义会员信息类目，会员卡激活后显示,包含name_type(name)和url字段
        @Protocol(fieldType = FieldType.STRING, order = 12, description = "自定义会员信息类目，会员卡激活后显示,包含name_type(name)和url字段。JSON结构")
        private String customField1;
        //        custom_field2            	否            	JSON结构            	自定义会员信息类目，会员卡激活后显示，包含name_type(name)和url字段
        @Protocol(fieldType = FieldType.STRING, order = 13, description = "自定义会员信息类目，会员卡激活后显示,包含name_type(name)和url字段。JSON结构")
        private String customField2;
        //        custom_field3            	否            	JSON结构            	自定义会员信息类目，会员卡激活后显示，包含name_type(name)和url字段
        @Protocol(fieldType = FieldType.STRING, order = 14, description = "自定义会员信息类目，会员卡激活后显示,包含name_type(name)和url字段。JSON结构")
        private String customField3;
//        custom_cell1            	否            	JSON结构            	自定义会员信息类目，会员卡激活后显示。

        @Protocol(fieldType = FieldType.STRING, order = 15, description = "自定义会员信息类目，会员卡激活后显示。name/tips/url。JSON结构")
        private String customCell1;
        //        bonus_rule            	        否            	JSON结构 	积分规则结构体，用于微信买单功能
        @Protocol(fieldType = FieldType.STRING, order = 16, description = "积分规则。JSON结构")
        private String bonusRule;
        //        activate_url            	是            	string（128）            	激活会员卡的url。
        @Protocol(fieldType = FieldType.STRING, order = 17, description = "激活会员卡的url。")
        private String activateUrl;
        //        bonus_cleared            	否            	string（128）            	积分清零规则。
        @Protocol(fieldType = FieldType.STRING, order = 18, description = "积分清零规则。")
        private String bonusCleared;
        //        balance_rules            	否            	string（128）            	储值说明。
        @Protocol(fieldType = FieldType.STRING, order = 19, description = "储值说明。")
     ...
     */
    @Data
    class CardBaseInfoResDataList extends ResultInfo {
        @Protocol(fieldType = FieldType.OBJECT, order = 2, description = "会员卡信息List")
        private List<CardBaseInfoResData> baseInfoList;
        @Protocol(fieldType = FieldType.INT, order = 3, description = "起始索引")
        private Integer pageNO;
        @Protocol(fieldType = FieldType.INT, order = 4, description = "每页结果数")
        private Integer pageSize;
    ...
    class CardBaseInfoResData extends ResultInfo {
        ...
        @Protocol(fieldType = FieldType.STRING, order = 8, description = "优惠券类型")
        private String cardType;
        //        logo_url 	             是 	string(128) 	http://mmbiz.qpic.cn/ 	卡券的商户logo，建议像素为300*300。
        @Protocol(fieldType = FieldType.STRING, order = 9, description = "卡券的商户logo，建议像素为300*300")
        ...
        @Protocol(fieldType = FieldType.STRING, order = 10, description = "Code展示类型")
        private String codeType;
        //        color 	             是 	string（16） 	Color010 	券颜色。按色彩规范标注填写Color010-Color100。详情见获取颜色列表接口
        @Protocol(fieldType = FieldType.STRING, order = 11, description = "券颜色。按色彩规范标注填写Color010-Color100。详情见获取颜色列表接口")
        private String color;
        //        brand_name             是 	string（36） 	海底捞 	商户名字,字数上限为12个汉字。
        @Protocol(fieldType = FieldType.STRING, order = 12, description = "商户名字,字数上限为12个汉字")
        private String brandName;
        //        notice 	             是 	string（48） 	请出示二维码核销卡券 	卡券使用提醒，字数上限为16个汉字。
        @Protocol(fieldType = FieldType.STRING, order = 13, description = "卡券使用提醒，字数上限为16个汉字")
        private String notice;
        //        description            是 	string（3072） 	不可与其他优惠同享/n如需团购券发票，请向店员提出要求。 	卡券使用说明，字数上限为1024个汉字。
        @Protocol(fieldType = FieldType.STRING, order = 14, description = "卡券使用说明，字数上限为1024个汉字")
        private String description;
        //        -sku              是 	int 	        100000   	卡券库存的数量，不支持填写0，上限为100000000。
        @Protocol(fieldType = FieldType.INT, order = 15, description = "库存信息")
        private Integer sku;
        //        date_info              是 	Json结构 	    见上述示例。 	使用日期，有效期的信息。
        @Protocol(fieldType = FieldType.STRING, order = 16, description = "使用日期，有效期的信息")
        private String dateInfo;
        //        use_custom_code        否 	bool 	        true 	是否自定义Code码。填写true或false，默认为false。通常自有优惠码系统的开发者选择自定义Code码，在卡券投放时带入。
        @Protocol(fieldType = FieldType.BOOL, order = 17, description = "是否自定义Code码.填写true或false，默认为false。通常自有优惠码系统的开发者选择自定义Code码，在卡券投放时带入。")
        private boolean useCustomCode;
        //        bind_openid 	         否 	bool 	        true 	是否指定用户领取，填写true或false。默认为false。
        @Protocol(fieldType = FieldType.BOOL, order = 18, description = "是否指定用户领取，填写true或false。默认为false。")
        private boolean bindOpenid;
        //        service_phone 	     否 	string（24） 	40012234 	客服电话。
        @Protocol(fieldType = FieldType.STRING, order = 19, description = "客服电话。")
...
//        source 	             否 	string（36） 	大众点评 	第三方来源名，例如同程旅游、大众点评。
        @Protocol(fieldType = FieldType.STRING, order = 21, description = "第三方来源名，例如同程旅游、大众点评。")
        private String source;
        //        custom_url_name 	     否 	string（15） 	立即使用 	自定义跳转外链的入口名字。
        @Protocol(fieldType = FieldType.STRING, order = 22, description = "自定义跳转外链的入口名字。")
        private String customUrlName;
        //        custom_url 	         否 	string（128） 	"xxxx.com" 	自定义跳转的URL。
        @Protocol(fieldType = FieldType.STRING, order = 23, description = "自定义跳转的URL。")
        private String customUrl;
        //        custom_url_sub_title 	 否 	string（18） 	更多惊喜 	显示在入口右侧的提示语。
        @Protocol(fieldType = FieldType.STRING, order = 24, description = "显示在入口右侧的提示语。")
        private String customUrlSubTitle;
        //        promotion_url_name 	 否 	string（15） 	产品介绍 	营销场景的自定义入口名称。
        @Protocol(fieldType = FieldType.STRING, order = 25, description = "营销场景的自定义入口名称。")
        private String promotionUrlName;
        //        promotion_url 	     否 	string（128） 	XXXX.com 	入口跳转外链的地址链接。
        @Protocol(fieldType = FieldType.STRING, order = 26, description = "入口跳转外链的地址链接。")
        private String promotionUrl;
        //        promotion_url_sub_title否 	string（18） 	卖场大优惠。 	显示在营销入口右侧的提示语。
        @Protocol(fieldType = FieldType.STRING, order = 27, description = "显示在营销入口右侧的提示语。")
        ...
        @Protocol(fieldType = FieldType.BOOL, order = 31, description = "填写true为用户点击进入会员卡时推送事件，默认为false。会员卡专用")
        ...
        @Protocol(fieldType = FieldType.STRING, order = 32, description = "填入 GET_CUSTOM_CODE_MODE_DEPOSIT 表示该卡券为预存code模式卡券，须导入超过库存数目的自定义code后方可投放，填入该字段后，quantity字段须为0,须导入code后再增加库存")
        private String customCodeMode;
        //        use_all_locations     否	bool	        true	设置本卡券支持全部门店，与location_id_list互斥
        @Protocol(fieldType = FieldType.BOOL, order = 33, description = "设置本卡券支持全部门店，与location_id_list互斥")
        private boolean useAllLocations;
        //        center_title	        否	string（18）	立即使用卡券顶部居中的按钮，仅在卡券状态正常(可以核销)时显示
        @Protocol(fieldType = FieldType.STRING, order = 34, description = "立即使用卡券顶部居中的按钮，仅在卡券状态正常(可以核销)时显示")
        private String centerTitle;
        //        center_sub_title	    否	string（24）	立即享受优惠显示在入口下方的提示，仅在卡券状态正常(可以核销)时显示。
        @Protocol(fieldType = FieldType.STRING, order = 35, description = "立即享受优惠显示在入口下方的提示，仅在卡券状态正常(可以核销)时显示。")
        private String centerSubTitle;
        //        center_url	        否	string（128）	www.qq.com顶部居中的url，仅在卡券状态正常(可以核销)时显示。
        @Protocol(fieldType = FieldType.STRING, order = 36, description = "顶部居中的url，仅在卡券状态正常(可以核销)时显示。")
        private String centerUrl;
        //        use_limit	            否	int	            100	每人可核销的数量限制,不填写默认为50。
        @Protocol(fieldType = FieldType.STRING, order = 37, description = "每人可核销的数量限制,不填写默认为50。")
        private Integer useLimit;
        @Protocol(fieldType = FieldType.BOOL, order = 38, description = "是否设置该会员卡支持拉出微信支付刷卡界面")
